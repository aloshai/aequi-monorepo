...
export class UniswapV3Adapter extends BaseDexAdapter {
  ...
  async computeV3Quote(params: V3QuoteParams): Promise<PriceQuote | null> {
    ...
    if (!dex.quoterAddress) {
      console.warn(`[UniswapV3] Missing quoter address for DEX ${dex.id}`)
      return null
    }
    ...
    try {
      ...
      console.warn(`[UniswapV3] Quoter call failed for ${tokenIn.symbol}->${tokenOut.symbol}:`, (error as Error).message)
      return null
    }
  }
}
